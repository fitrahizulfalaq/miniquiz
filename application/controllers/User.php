...
class User extends CI_Controller {
    ...
    public function tambah()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //CEK GAMBAR
            $config['upload_path']          = 'assets/dist/img/fotouser/';
    ...
    {           
        ...
        if ($this->session->userdata('tipe_user') < $this->fungsi->pilihan_selected("tb_user",$id)->row("tipe_user")) {
            $this->session->set_flashdata('warning','Tidak Bisa Mengedit Kategori User diatasnya');
            redirect('user');
        } elseif ($this->session->userdata('tipe_user') < 2 and $this->session->userdata('id') != $id) {
            $this->session->set_flashdata('warning','Anda Hanya Bisa Mengedit Profil Anda Sendiri');
            ...
            if (@$_FILES['foto']['name'] != null) {
                if ($this->upload->do_upload('foto')) {
                    ...
                    if ($itemfoto->foto != null) {
                        $target_file = 'assets/dist/img/fotouser/'.$itemfoto->foto;
                        unlink($target_file);
                    }
